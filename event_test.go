...
func TestEventListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Events.List(context.TODO(), m3ter.EventListParams{
		...
		IncludeActioned:  m3ter.F(true),
		NextToken:        m3ter.F("nextToken"),
		NotificationCode: m3ter.F("notificationCode"),
		NotificationID:   m3ter.F("notificationId"),
		PageSize:         m3ter.F(int64(1)),
		ResourceID:       m3ter.F("resourceId"),
	})
	if err != nil {
		var apierr *m3ter.Error
		if errors.As(err, &apierr) {
...
func TestEventGetFieldsWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
...
func TestEventGetTypes(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
